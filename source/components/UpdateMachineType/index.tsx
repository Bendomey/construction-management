...
import { UPDATE_MACHINE_TYPE_ACTION } from "../../state/slices/app.slice";
import _ from 'lodash'
import { MachineType, MachineTypeAttribute } from "../../../models";
...
export const UpdateMachineType = ({ isOpen, close, data, index }: Props) => {
    ...
    const handleSubmit = (values: FormFieldsParams, helpers: FormikHelpers<FormFieldsParams>) => {
        ...
        dispatch(UPDATE_MACHINE_TYPE_ACTION({
            index,
            data: {
                id: data.id,
                name: values.name,
    ...
    const attributeInit = useMemo(() => {
        return data.attributes.map(attr => ({
            ...attr,
            isTitleAttribute: attr.name === data.metaData.titleAttribute
        }))
    }, [data])

    return (
        <Formik
            initialValues={{ name: data.name, attributes: attributeInit }}
                    ...
                    return (
                        <Modal
                            ...
                            <Modal.Content h="full">
                                <Modal.CloseButton />
                                <Modal.Header>Update Machine Type</Modal.Header>
                                <Modal.Body>
                                    <Text mb={1}>Name *</Text>
